...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
class PERSON {
  constructor(name, type, collisionBox, model) {
...
if (this.type === 'player') {
  if (this.developerView) {
    if (this.currentWeapon === 'undefined') {
      this.currentWeapon.type = 'undefined';
    };
    this.collisionBox.material.visible = true;
    this.collisionBox.material.transparent = true;
    this.collisionBox.material.opacity = 0.25;
    document.getElementById("dev_tools").innerHTML = `<p><a class="data">Developer Info:</a></p>
    <p><a class="data">Coordinates (XYZ): ` + this.collisionBox.position.x +  ` / ` + this.collisionBox.position.y +  ` / ` + this.collisionBox.position.z + `</a></p>
    <p><a class="data">Velocities (XYZ): ` + this.xVelocity + ` / ` + this.yVelocity + ` / ` + this.zVelocity + `</a></p>
    <p><a class="data">Selected Weapon: ` + ((this.currentWeapon) ? (this.currentWeapon.type) : ('undefined')) + `</a></p>`
    ;
        ...
        for (let i=0; i < collisionMap.length; i++) {
          ...
          //if not on top of the colision box, then check if you are coliding with the colision box, if you are, put yourself on top of the colision box and stop falling.
            ...
            if (Math.sign(this.yVelocity) <= 0) {
              ...
              console.log('position corrected, ' + this.name + 's y coordinate is now ' + this.collisionBox.position.y);
              updateBoundingBox(this.collisionBox);
              this.midAir = false;
              this.yVelocity = 0;
...
//for sean debugging
  ...
  for (let i=0; i < collisionMap.length; i++) {
    ...
    //if not on top of the colision box, then check if you are coliding with the colision box, if you are, put yourself on top of the colision box and stop falling.
      ...
      updateBoundingBox(this.collisionBox);
      console.log('position corrected, ' + this.name + 's x coordinate is now ' + this.collisionBox.position.x);
    }
  };
...
class PISTOL {
  constructor(user) {
    ...
    this.user = user; // i dont get this...
  ...
  if (this.user.LmousePressed) {
    this.shooting = true;
  } else {
    this.shooting = false;
  }
  if (this.user.fPressed) {
...
//~~~~~~~
let houseFloor;
scene.add(houseFloor = new THREE.Mesh(
    new THREE.BoxGeometry(3, 0.1, 3),
    new THREE.MeshPhongMaterial({color: 0x696969, side: THREE.DoubleSide})
));
    houseFloor.geometry.computeBoundingBox();
    houseFloor.boundingBox = houseFloor.geometry.boundingBox.clone();

    houseFloor.position.y = houseFloor.geometry.parameters.height/2; houseFloor.position.x = 2;
    houseFloor.position.x = -7;
    houseFloor.position.z = -5;

    updateBoundingBox(houseFloor);

    collisionMap.push(houseFloor);
//~~~~~~~
let houseWall1;
scene.add(houseWall1 = new THREE.Mesh(
    new THREE.BoxGeometry(0.1, 1.5, 3),
    new THREE.MeshPhongMaterial({color: 0x696969, side: THREE.DoubleSide})
));
    houseWall1.geometry.computeBoundingBox();
    houseWall1.boundingBox = houseWall1.geometry.boundingBox.clone();

    houseWall1.position.y = houseWall1.geometry.parameters.height/2; houseWall1.position.x = 2;
    console.log(houseFloor); //?? why cant it refer to other declared scene items?
    houseWall1.position.x = -7;
    houseWall1.position.z = -5;

    updateBoundingBox(houseWall1);

    collisionMap.push(houseWall1);
//~~~~~~~
let houseWall2;
scene.add(houseWall2 = new THREE.Mesh(
    new THREE.BoxGeometry(3, 1.5, 0.1),
    new THREE.MeshPhongMaterial({color: 0x696969, side: THREE.DoubleSide})
));
    houseWall2.geometry.computeBoundingBox();
    houseWall2.boundingBox = houseWall2.geometry.boundingBox.clone();

    houseWall2.position.y = houseWall2.geometry.parameters.height/2; houseWall2.position.x = 2;
    houseWall2.position.x = -7;
    houseWall2.position.z = -5;

    updateBoundingBox(houseWall2);

    collisionMap.push(houseWall2);
//~~~~~~~
let houseRoof;
scene.add(houseRoof = new THREE.Mesh(
    new THREE.BoxGeometry(3, 0.1, 3),
    new THREE.MeshPhongMaterial({color: 0x696969, side: THREE.DoubleSide})
));
    houseRoof.geometry.computeBoundingBox();
    houseRoof.boundingBox = houseRoof.geometry.boundingBox.clone();

    houseRoof.position.y = 1.5 + houseRoof.geometry.parameters.height/2; houseRoof.position.x = 2;
    houseRoof.position.x = -7;
    houseRoof.position.z = -5;

    updateBoundingBox(houseRoof);

    collisionMap.push(houseRoof);
//~~~~~~~
let light;
...
let phongie = new PERSON('phongie', 'player', (
  new THREE.Mesh(
    new THREE.BoxGeometry(0.5, 0.5, 0.5),
    new THREE.MeshPhongMaterial({color: 0x754c1c, side: THREE.DoubleSide})
)), (
...
function render(time) { // the game run loop, stuff that happens every game frame. hi!
    ...
    bot1.gameLoop(); //stuff related to the bot instances
    ...
    renderer.render(scene, phongie.useThirdPerson ? bot1.camera : phongie.camera);
